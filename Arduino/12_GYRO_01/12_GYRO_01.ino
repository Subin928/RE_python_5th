...
void loop() {
  ...
  Wire.requestFrom(MPU, 14, true); // 실제 데이터 읽기 시작
    ...
    하지만 아두이노 I2C 통신은 한 번에 1바이트만 통신이 가능
    그래서 1바이트씩 두 번 값을 읽어와 두 바이트를 합쳐서 저장

    <<: 비트를 왼쪽으로 이동시키는 연산자
      x << n
      x의 비트를 왼쪽으로 n칸 이동시키고, 오른쪽 빈 비트에는 0을 채움
        0010010 <<  2 => 01001000 이렇게 바뀜
    ...
    */
    ...
    데이터를 읽어오는 지점을 지정했고, 해당 지점부터 XYZ축 가속도, 센서온도, 자이로스코프값이 순서대로 오기 때문
    시작 주소와 바이트 수를 정확하게 맞춰서 오기 때문에
    데이터를 읽어왔을 때 순서가 바뀌거나 랜덤하게 저장될 일 없이 사용 가능
    */
