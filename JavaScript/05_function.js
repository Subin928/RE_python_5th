...
// 함수 표현식, 화살표함수는 호출 불가
console.log(
    '사각형의 넓이 구하기 (함수 선언식, 선언보다 위에서 호출)',
    myFunc('어라라')
); // 실행됨

// myFunc3(); // 함수 표현식, 선언보다 위에서 호출 // 실행 안됨
// console.log(
//     "사각형의 넓이 구하기 (화살표 함수, 선언보다 위에서 호출),"
//     square(10, 8),
//     myFunc('어라라')); // 실행 안됨
...
myFunc("이거 콘솔에 찍히지 않아요"); // console.log() 사용 안했으니까 그냥 냅다 "이거 콘솔에 찍히지 않아요"를 쓴 것
console.log(myFunc("이렇게 해야 정상적으로 콘솔에 찍혀요"));
...
const arrowFunc = () => {
    // 화살표 함수는 {}안에 꼭 return이 포함되어야 함
    return '이렇게 화살표 함수를 사용할 수 있어요';
};

// 한 줄로 작성 + 괄호 없는 경우네는 해당 내용을 자동으로 return
...
console.log(noReturnFunc(5, 7)); // return 하지 않으면 그냥 함수 스코프 내부 코드 실행되고 끝남